...
- **design** (aliases: brief, brand): Hybrid design workflow (Codex Design import path A or code-based skill path B)
...
Purpose: Produce web/brand design artifacts via Codex Design import (path A) or code-based skill pipeline (path B). Integrates brand context from `.moai/project/brand/` and design briefs from `.moai/design/`.
