...
| "I can put all logic in AGENTS.md, rules are overkill" | AGENTS.md has a 40K character limit. Rules load conditionally and scale without bloating the prompt. |
...
| "This skill does not need allowed-tools, Codex will figure it out" | Missing allowed-tools means the skill silently inherits all tools. Explicit is safer than implicit. |
...
- AGENTS.md exceeds 40,000 characters
...
- [ ] AGENTS.md character count is under 40,000 (show wc -c output)
- [ ] settings.json is valid JSON (show json validation output)
- [ ] Every hook in settings.json has a matching script file in .Codex/hooks/
