...
|---|---|
| "I will add the template file directly to .Codex/, not to internal/template/templates/" | Template-first rule is HARD. Local files are overwritten on moai update. Edit the template source. |
...
- New file added to `.Codex/` or `.moai/` without a corresponding file in `internal/template/templates/`
...
- [ ] Every new file under `.Codex/`, `.moai/`, `.agency/` has a corresponding template source
