...
describe("programCreateSchema (FR-001, NFR-003)", () => {
  ...
  it("status 입력은 생성 스키마 결과에서 제외한다", () => {
    const result = programCreateSchema.safeParse({
      title: "x",
      priceKrw: 1,
      status: "CLOSED",
...
describe("programUpdateSchema (FR-006)", () => {
  it("부분 필드만으로 통과한다", () => {
    expect(programUpdateSchema.safeParse({ title: "수정된 프로그램" }).success).toBe(true);
  });
  ...
  it("status 단독 입력은 수정할 필드가 없어 실패한다", () => {
    expect(programUpdateSchema.safeParse({ status: "CLOSED" }).success).toBe(false);
  });
});
